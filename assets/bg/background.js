(() => {
    // If the pen code assumes elements already in the DOM:
    const start = () => {
      /*          *     .        *  .    *    *   . 
...
const computeStarCount = () => Math.round((window.innerWidth + window.innerHeight) / 8);
let STAR_COUNT = computeStarCount();

const canvas = document.getElementById('bgcanvas');
if (!canvas) return; // background canvas not present
...
function resize() {
  ...
  // keep CSS size in sync (helps when styles change)
  canvas.style.width = window.innerWidth + 'px';
  canvas.style.height = window.innerHeight + 'px';

  // adjust star pool to new target count
  const target = computeStarCount();
